        ...
        if len(buffer_frames) >= SIGN_BUFFER_SIZE:
            ...
            if processed_frame != []:
                if not has_sign_started:
                    ...
                    print(f'Start Video: {len(buffer_frames)}')

                # Add relevant start and middle frames
                frames.extend(buffer_frames)
                print(f'Start Extension {len(buffer_frames)}')
            else:
                ...
                if has_sign_started:        
                    ...
                    temp_buffer_frames = buffer_frames[:end_frame_ind]    

                    # Mark sign as complete if 3+ frames have no landmarks
                    if SIGN_BUFFER_SIZE - len(temp_buffer_frames) > END_SIGN_BUFFER:
                        is_sign_complete = True
                        has_sign_started = False

                        # Add relevant end frames
                        frames.extend(temp_buffer_frames)
                        print(f'End video  {len(temp_buffer_frames)}')
                    else:
                        # Add relevant middle frames
                        frames.extend(buffer_frames)
                        print(f'Middle extension {len(buffer_frames)}')
