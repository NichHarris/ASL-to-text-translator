...
def augment_video(seq, frames, curr_folder, aug_num, fps):
    ...
    # Save video 
    out = cv2.VideoWriter(f"{DATA_PATH}/{curr_folder}/aug_{aug_num}.mp4", video_writer, fps, (width, height))
...
from util import vid_iterator, time_decorator
...
vid_fn = vid_iterator(DATA_PATH, action_fn, video_fn)
time_decorator(vid_fn, '\nTotal Video Augmentation Time (s):')
'''