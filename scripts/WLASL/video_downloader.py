...
def download_nonyt_videos(indexfile, saveto='../../files/raw_videos'):
...
def download_yt_videos(indexfile, saveto='../../files/raw_videos'):
    content = json.load(open(indexfile))
    ...
    for entry in content:
        ...
        for inst in instances:
            ...
            if os.path.exists(os.path.join(saveto, video_id + '.mp4')) or os.path.exists(os.path.join(saveto, video_id + '.mkv')):
                ...
                cmd = cmd.format(video_url, saveto + "/" + video_id)
